        ...
        build: function (data) {
            ...
            Object.keys(data).forEach(data_id => {
                ...
                if (data_id == 'img') {
                    ...
                    img.className = data_id;
                    img.setAttribute('src', data_value);
                    card.append(img);
                } else {
                    let span = document.createElement('span');
                    span.className = data_id;
                    span.innerHTML = data_value;

                    if (data_id == 'in_stock') {
                        if (data_value <= 0) {
                            span.className = data_id + ' unavailable'
                            span.innerHTML = 'OUT OF STOCK';
                        } else {
                            span.className = data_id + ' available';
                            span.innerHTML = 'IN STOCK: (' + data[data_id] + ')';
                        }
                    }

                    if (data_id == 'price' && data.discount > 0) {
                        span.className += ' striked';
                    }

                    if (data_id == 'discount') {
                        let discounted = data.price - (data.price / 100 * data.discount);
                        span.innerHTML = 'Current price: ' + discounted.toFixed(2) + ' Eur ';
            ...
            btn_container.className = 'buttons';
        ...
        edit: {
            ...
            onClickListener: function (e) {
                if (e.target.className === 'edit') {
                    ...
                    let tr = e.target.closest('div.product-card-container');
